...
function PrizeLadder({ streak, bestPrizeStreak }: { streak: number; bestPrizeStreak: number }) {
  const cards = [{ streak: 0, name: GUARANTEED_PRIZE.name, retail: GUARANTEED_PRIZE.retail, label: GUARANTEED_PRIZE.label }, ...PRIZE_TIERS];
  ...
  return <section className="panel"><div className="eyebrow">Prize Ladder</div><h2>Highest unlocked bonus prize wins</h2><p className="notice">Bonus prizes are not cumulative. Each higher tier replaces the previous bonus prize.</p><div className="ladder">{cards.map((tier) => {
    ...
    return <article className={`prize ${unlocked?'unlocked':''} ${next?'next':''}`} key={`${tier.streak}-${tier.name}`}><span className="pill">{tier.streak===0?'Entry':`${tier.streak} wins`}</span><h3>{tier.name}</h3><p>{tier.label}</p><p><strong>Retail:</strong> {money(tier.retail)}</p>{unlocked && <p style={{color:'var(--green)',fontWeight:900}}>Unlocked</p>}</article>;
  })}</div></section>;
}
